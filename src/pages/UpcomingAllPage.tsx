...
import { fetchUpcomingAnime } from '../api/jikan'
import { useStore } from '../store/useStore'
import AnimeCard from '../components/AnimeCard'
import { GridSkeleton } from '../components/Skeleton'

export default function UpcomingAllPage() {
    ...
    <div className="min-h-screen bg-bg">
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="font-display font-700 text-2xl text-text-base">Upcoming Anime</h1>
            <p className="text-text-muted text-sm">Anime not yet aired · Add to your watchlist</p>
          </div>
        </div>
        ...
        {loading && anime.length === 0 ? <GridSkeleton count={24} /> : (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              {anime.map(a => <AnimeCard key={a.mal_id} anime={a} isUpcoming />)}
            </div>
